...
export default function Header({history}) {
  ...
  return (
    <div className="header">
      ...
      <h2 data-testid="page-title">{/comidas/.test(history.location.pathname) ? 'Comidas' : 'Bebidas' }</h2>
      <input type="image" data-testid="search-top-btn" src={searchTopBtn} alt="search top button" onClick={() => setVisibleSearch(!visibleSearch)}/>
      {visibleSearch && <form>
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} data-testid="search-input" />
